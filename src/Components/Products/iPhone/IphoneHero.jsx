...
export const IPhoneHero = ({ id, img, imgMV, titaniumImg, title, New, pricing, ordering, link, color}) => {
  ...
  return (
    ...
    {!isPageWide ? <img className="card-iphone-img" src={img} alt={title}/> : <img className="card-iphone-img" src={imgMV} alt={title}/> }
    <div className="card-details card-details-iphone">
      <span>{New}</span>
      <h1 style={{color}} className="card-title iphone-h1">
        {title}
      </h1>
      {titaniumImg ? <img className="titanium-img" src={titaniumImg} alt="title showing the text 'titanium'"/> :
      <h2 style={{color, fontSize: "48px", fontWeight: '600'}}>Newphoria</h2>}
